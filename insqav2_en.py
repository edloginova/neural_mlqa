...
model_name = "torch_insqav2_en"
torch.utils.backcompat.broadcast_warning.enabled = True

# # Loading data

corpus_dir = "data/processed/insuranceqa-v2/en/"
data_q_file = "InsuranceQA.question.anslabel.token.encoded"
qa_pairs = pd.read_csv(corpus_dir + data_q_file, sep='\t', header=None, names=['domain', 'question', 'answer_ids'],
...
answers = pd.read_csv(corpus_dir + data_a_file, sep='\t', header=None, names=['answer_id', 'answer'],
...
for option in ['train', 'test', 'valid']:
    file_tmp = corpus_dir + "InsuranceQA.question.anslabel.token." + str(
        K_TEST) + ".pool.solr." + option + ".encoded"
    df_tmp = pd.read_csv(file_tmp, sep='\t', header=None, names=['domain', 'question', 'answer_ids', 'pool'])
    if option != 'valid':
...
data = full_qa_pairs.merge(split_tmp_expanded.drop(axis=1, labels=['domain']), on=['question', 'answer_id'],
                           how='inner')
...
fasttext_embeddings = FastText.load_fasttext_format(EMBEDDING_PATH + 'cc.en.300.bin')
