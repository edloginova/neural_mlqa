...
epochs = 20
batch_size = 20
model_name = 'keras_semeval_ar_subtaskD'
...
synonyms = pickle.load(open(EXTRA_DATA_PATH + 'arabic_synonyms.p', 'rb'))
not_found = pickle.load(open(EXTRA_DATA_PATH + 'arabic_not_found.p', 'rb'))
embeddings_index = FastText.load_fasttext_format(EMBEDDING_PATH + 'cc.ar.300.bin')
...
for word, i in vocabulary.items():
    if word in embeddings_index.wv.vocab:
        ...
        if word in synonyms.keys():
            if synonyms[word] in embeddings_index.wv.vocab:
                embedding_vector = embeddings_index[synonyms[word]]
                embedding_matrix[i] = embedding_vector
            else:
